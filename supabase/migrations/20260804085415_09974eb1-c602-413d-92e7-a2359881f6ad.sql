CREATE OR REPLACE FUNCTION public.queue_event_email(p_org uuid, p_event_key text, p_subject text, p_message text, p_action_url text, p_roles text[] DEFAULT '{}'::text[], p_user_ids uuid[] DEFAULT '{}'::uuid[])
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF array_length(p_roles, 1) > 0 THEN
    SELECT array_agg(DISTINCT lower(replace(replace(r, ' ', '_'), '-', '_')))
      INTO v_norm_roles FROM unnest(p_roles) r;

    -- built-in app roles: every user holding the role
    SELECT array_agg(DISTINCT pr.email) INTO v_recipients
     ...
     WHERE pr.organization_id = p_org
       ...
       AND lower(replace(replace(ur.role::text, ' ', '_'), '-', '_')) = ANY(v_norm_roles);
    v_recipients := COALESCE(v_recipients, '{}');

    -- custom roles with an equivalent name: every user holding that role
    v_recipients := v_recipients || COALESCE((
      SELECT array_agg(DISTINCT pr.email)
        FROM public.profiles pr
        JOIN public.user_custom_roles ucr ON ucr.user_id = pr.user_id
        JOIN public.roles rl ON rl.id = ucr.role_id
       WHERE pr.organization_id = p_org
         AND pr.email IS NOT NULL
         AND (rl.organization_id = p_org OR rl.organization_id IS NULL)
         AND lower(replace(replace(rl.name, ' ', '_'), '-', '_')) = ANY(v_norm_roles)
    ), '{}'::text[]);
  END IF;
  v_recipients := COALESCE(v_recipients, '{}');

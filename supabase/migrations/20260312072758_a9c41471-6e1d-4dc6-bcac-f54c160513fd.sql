...
-- Add expense_account_id to ap_invoice_lines so each line can target a specific GL account
ALTER TABLE public.ap_invoice_lines 
ADD COLUMN expense_account_id UUID REFERENCES public.gl_accounts(id);

-- Update the GL posting trigger to use the per-line expense_account_id

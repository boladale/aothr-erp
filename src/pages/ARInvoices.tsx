...
export default function ARInvoices() {
  ...
  const updateLine = (i: number, field: keyof InvoiceLine, value: string) => {
    ...
    if (field === 'item_id') {
      ...
      if (item) {
        updated[i].unit_price = String(item.unit_cost || 0);
        updated[i].description = item.name;
      }
    }
    setLines(updated);
  };
  ...
  const handleCreate = async () => {
    ...
    const lineInserts = lines.map(l => ({
      invoice_id: inv.id, item_id: l.item_id || null,
      description: l.description, quantity: parseFloat(l.quantity) || 1,
      unit_price: parseFloat(l.unit_price) || 0,
      revenue_account_id: l.revenue_account_id || null,
    }));
    
    const { error: lineErr } = await supabase.from('ar_invoice_lines').insert(lineInserts);
                      ...
                      {lines.map((line, i) => (
                        <div key={i} className="grid grid-cols-12 gap-2 items-end">
                          ...
                          <div className="col-span-3">
                            ...
                            <Input className="h-9" value={line.description} onChange={e => updateLine(i, 'description', e.target.value)} />
                          </div>
                          <div className="col-span-1">
                            {i === 0 && <Label className="text-xs">Qty</Label>}
                            <Input className="h-9" type="number" value={line.quantity} onChange={e => updateLine(i, 'quantity', e.target.value)} />
                          </div>
                          <div className="col-span-2">
                            {i === 0 && <Label className="text-xs">Unit Price</Label>}
                            <Input className="h-9" type="number" value={line.unit_price} onChange={e => updateLine(i, 'unit_price', e.target.value)} />
                          </div>
                          <div className="col-span-2">
                            ...
                            <Select value={line.revenue_account_id || 'default'} onValueChange={v => updateLine(i, 'revenue_account_id', v === 'default' ? '' : v)}>
                              ...
                              <SelectContent>
                                <SelectItem value="default">Default (4100)</SelectItem>
                                {revenueAccounts.map(a => <SelectItem key={a.id} value={a.id}>{a.account_code}</SelectItem>)}
                              </SelectContent>
                            </Select>
                          </div>
                          <div className="col-span-1">
                            {lines.length > 1 && <Button variant="ghost" size="sm" className="h-9" onClick={() => removeLine(i)}>×</Button>}
                          </div>
                        </div>
                      ))}
                  ...
                  {filtered.map(inv => (
                    <tr key={inv.id} className="hover:bg-muted/50">
                      ...
                      <td className="px-4 py-2.5">
                        <Badge variant="outline" className={paymentStatusColors[inv.payment_status] || ''}>
                          {inv.payment_status}
                        </Badge>
                      </td>
                      {canManage && (
                        <td className="px-4 py-2.5">
                          {inv.status === 'draft' && (
                            <Button variant="outline" size="sm" onClick={() => handlePost(inv.id)}>

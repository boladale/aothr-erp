...
export default function JournalEntries() {
  ...
  const generateEntryNumber = () => {
    const now = new Date();
    return `JE-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`;
  };
  ...
  const handleCreate = async () => {
    ...
    const { error: lineError } = await supabase.from('gl_journal_lines').insert(lineInserts);
    ...
    <AppLayout>
      <div className="page-container">
        <PageHeader
          ...
          description="Record and manage general ledger transactions"
          actions={canManage ? (
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-4">
                      ...
                      <tbody>
                        {lines.map((line, i) => (
                          <tr key={i} className="border-b">
                            ...
                            <td className="px-2 py-1">
                              <Input type="number" className="h-9 text-right" value={line.debit || ''} onChange={e => setLines(l => l.map((ll, idx) => idx === i ? { ...ll, debit: parseFloat(e.target.value) || 0, credit: parseFloat(e.target.value) > 0 ? 0 : ll.credit } : ll))} />
                            </td>
                            <td className="px-2 py-1">
                              <Input type="number" className="h-9 text-right" value={line.credit || ''} onChange={e => setLines(l => l.map((ll, idx) => idx === i ? { ...ll, credit: parseFloat(e.target.value) || 0, debit: parseFloat(e.target.value) > 0 ? 0 : ll.debit } : ll))} />
                            </td>
                            <td className="px-2 py-1">
                              <Input className="h-9" value={line.description} onChange={e => setLines(l => l.map((ll, idx) => idx === i ? { ...ll, description: e.target.value } : ll))} />
                            </td>
                            <td className="px-2 py-1">
                              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => removeLine(i)} disabled={lines.length <= 2}><Trash2 className="h-3 w-3" /></Button>
                            </td>
                          </tr>
                        ))}
                        <tr className="font-semibold bg-muted/30">
                          ...
                          <td className="px-2 py-2">
                            {isBalanced ? (
                              ...
                              <span className="text-destructive text-xs">✗ Difference: {formatCurrency(Math.abs(totalDebit - totalCredit))}</span>
                            )}
                          </td>
                          <td></td>
                        </tr>
                  ...
                  <Button onClick={handleCreate} className="w-full" disabled={!isBalanced}>Create Journal Entry</Button>
                </div>
              </DialogContent>
            </Dialog>
          ) : undefined}
                    ...
                    <tr key={e.id} className="hover:bg-muted/50">
                      ...
                      <td className="px-4 py-3 text-right">
                        {e.status === 'draft' && canManage && (
                          <Button variant="outline" size="sm" onClick={() => handlePost(e.id)}>

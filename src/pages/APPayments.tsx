...
export default function APPayments() {
  ...
  const paymentsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('ap_payments').select('*, vendor:vendors(name, code)').order('created_at', { ascending: false });
  ...
  const vendorsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('vendors').select('id, name, code').eq('status', 'active').order('name');
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editingPayment) {
        ...
        return { number: editingPayment.payment_number, edit: true };
      } else {
        ...
        const { error: allocErr } = await supabase.from('ap_payment_allocations').insert(allocationRows);
        if (allocErr) throw allocErr;
        return { number: payNum, edit: false };
      }
    },
    onSuccess: (res) => {
  ...
  const postMutation = useMutation({
    mutationFn: async ({ id }: { id: string; number: string }) => {
      const { data, error } = await supabase.functions.invoke('secure-action', {
        body: { action: 'payment_post', payload: { id } },
      });
  ...
  return (
    <AppLayout>
      <div className="page-container">
        ...
        {paymentsQ.isLoading ? <Skeleton className="h-64" /> : (
          <Card><CardContent className="p-0"><DataTable columns={columns} data={paymentsQ.data || []} /></CardContent></Card>
        )}
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <DialogFooter>
              ...
              <Button onClick={handleSave} disabled={selectedInvoices.size === 0 || saveMutation.isPending}>
                {saveMutation.isPending ? 'Saving...' : (editingPayment ? 'Update Payment' : 'Create Payment')}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
